...
const DownloadOfferLetter = async (req, res) => {
  try {
    ...
    if (!applicant || !applicant.offerLetter) {
      return res.status(404).json({ msg: "No offer letter found for this student!" });
    }

    // Verify the student is requesting their own offer letter
    if (req.user.role === 'student' && req.user._id.toString() !== studentId) {
      return res.status(403).json({ msg: "Unauthorized to download this offer letter" });
    }

    // Redirect to Cloudinary URL for download
    res.redirect(applicant.offerLetter);
  } catch (error) {
    console.error(error);
    return res.status(500).json({ msg: "Server error", error });
  }
